...
$CFLAGS << " #{ENV["CFLAGS"]}" << `PKG_CONFIG_PATH=/opt/X11/lib/pkgconfig:$PKG_CONFIG_PATH pkg-config --cflags simple-tiles`.chomp << `gdal-config --cflags`.chomp
$LIBS << " #{ENV["LIBS"]}" << `PKG_CONFIG_PATH=/opt/X11/lib/pkgconfig:$PKG_CONFIG_PATH pkg-config --libs simple-tiles`.chomp << `gdal-config --libs`
